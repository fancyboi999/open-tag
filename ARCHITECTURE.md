...
- **No separate wake operation**: wake = server re-sends `agent:start` with the saved `sessionId`; the runtime resumes via `--resume` (claude), `thread/resume` (codex), `--session-id` (copilot, idempotent create-or-resume), or `--session` (opencode, with the captured `ses_…` id).
...
- `runtimes.ts` — Runtime **registry** (`REG` / `getRuntime()`) + `detectRuntimes()` probes which runtimes are installed locally (`claude` / `codex` / `copilot` / `kimi` / `opencode`). NB: `REG` implements `claude`, `codex`, `copilot`, `opencode`; `detectRuntimes()` still *advertises* `kimi` which has no adapter yet (selecting it → `no runtime` offline — see `docs/tech-debt-tracker.md`).
...
- `runtime.ts` interface — all differences between claude / codex / copilot / opencode (incl. persistent-process vs one-shot-per-turn lifecycles) are contained behind adapters; the upper-layer `agentManager` is runtime-agnostic.
