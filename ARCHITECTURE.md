...
- **No separate wake operation**: wake = server re-sends `agent:start` with the saved `sessionId`; the runtime resumes via `--resume` (claude), `thread/resume` (codex), `--session-id` (copilot, idempotent create-or-resume), `--session` (opencode, captured `ses_…` id), `-r` (kimi, captured `session_…` id), or `--session` (pi, captured UUID).
...
- `runtimes.ts` — Runtime **registry** (`REG` / `getRuntime()`) + `detectRuntimes()` probes which runtimes are installed locally (`claude` / `codex` / `copilot` / `kimi` / `opencode` / `pi`). `REG` implements all six; every probed runtime has an adapter (the earlier advertise-without-adapter drift is resolved — see `docs/tech-debt-tracker.md` I33).
...
- `runtime.ts` interface — all differences between claude / codex / copilot / opencode / kimi / pi (incl. persistent-process vs one-shot-per-turn lifecycles) are contained behind adapters; the upper-layer `agentManager` is runtime-agnostic.
