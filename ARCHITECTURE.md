...
- **@mentions are stored as structured `message_mentions` rows** (supports inbox queries) and are the single source of truth: the frontend linkifies `@name` from each message's own `mentions[]` (`messageRender.processMessageContent`), so an `@` the server never recorded (e.g. a non-member in a private channel / DM) renders as plain text rather than a fake clickable mention.
