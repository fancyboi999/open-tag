...
- **Status is two-dimensional** (daemon `agentManager` emits; `ws.ts` writes `agents.status/activity` to DB):
  ...
  - Machine offline: server forces `inactive / offline` when the daemon connection drops (`ws.ts` takes all agents on that machine offline)
...
- `socketio.ts` — Human-side real-time (socket.io, `/socket.io/`); `emitMapped` sends `message:new` (full message object) / `message:updated` / `task:created` / `task:updated` / `agent:activity` (status + trajectory combined) / `thread:updated` / `agent:created|deleted` / `machine:status`.
- `ws.ts` — **Daemon control plane** (bare WS, `/daemon/connect?key=`). Handles D→S: `ready` / `agent:status` / `agent:activity` / `agent:session` / `agent:trajectory` / `workspace:file_tree` / `workspace:file_content` / `skills:list`.
