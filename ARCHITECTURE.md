...
- **No separate wake operation**: wake = server re-sends `agent:start` with the saved `sessionId`; the runtime resumes via `--resume` (claude), `thread/resume` (codex), `--session-id` (copilot, idempotent create-or-resume), `--session` (opencode, captured `ses_…` id), or `-r` (kimi, captured `session_…` id).
...
- `runtimes.ts` — Runtime **registry** (`REG` / `getRuntime()`) + `detectRuntimes()` probes which runtimes are installed locally (`claude` / `codex` / `copilot` / `kimi` / `opencode`). `REG` implements all five; every probed runtime now has an adapter (the earlier `kimi` advertise-without-adapter drift is resolved — see `docs/tech-debt-tracker.md` I33).
...
- `runtime.ts` interface — all differences between claude / codex / copilot / opencode / kimi (incl. persistent-process vs one-shot-per-turn lifecycles) are contained behind adapters; the upper-layer `agentManager` is runtime-agnostic.
