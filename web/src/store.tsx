...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const reloadMembers = async () => {
    ...
    setMembersState((state) => state === "ready" ? "refreshing" : "loading");
    try {
      const [ag, hm] = await Promise.all([
        api("GET", "/api/agents"),
        api("GET", `/api/servers/${sid}/members`),
      ]);
      if (!Array.isArray(ag) || !Array.isArray(hm)) throw new Error("invalid members response");
      if (!fresh()) return;
      setAgents(ag);
      setHumans(hm);
