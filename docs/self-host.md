...
If you run the control plane on **Coolify**, use
[`docker-compose.coolify.yml`](../docker-compose.coolify.yml) as a **Docker Compose**
service in the Coolify web UI. The file is image-only, matching Coolify's compose
examples: the app service pulls `karlorz/open-tag:latest` instead of referencing
this repository's `Dockerfile`.

The file keeps the same open-tag topology (Postgres + Redis + app), but is
adjusted for Coolify's deployment model:

- No host `ports:` bindings — `SERVICE_URL_APP_7788` tells Coolify to route
  the generated/custom domain to the app container's internal port `7788`.
- No custom Docker networks — this stack is self-contained, so the default stack
  network is enough.
- Secrets are generated by Coolify magic variables (`SERVICE_PASSWORD_POSTGRES`,
  `SERVICE_HEX_64_JWT`, `SERVICE_HEX_64_DAEMON`, `SERVICE_HEX_64_ADMIN_SETUP`)
  and are visible/editable in the Coolify environment-variable UI.
- The **daemon still runs outside Coolify** on the machine that has your agent CLIs,
  code, and credentials.

The app image is built from the repository `Dockerfile` by
`.github/workflows/publish-app-image.yml` on pushes to `main`, then published to
Docker Hub as `karlorz/open-tag:latest` and to GHCR as
`ghcr.io/karlorz/open-tag:latest`. The Docker Hub publish path expects
