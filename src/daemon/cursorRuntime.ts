...
export function handleCursorEvent(evt: any): CursorEmit {
  ...
  if (evt?.type === "tool_call" && evt.subtype === "started") {
    ...
    if (tc && typeof tc === "object") {
      const key = Object.keys(tc)[0] ?? "tool"; // e.g. "shellToolCall" → "shell"
    ...
    else if (evt.result) out.trajectory.push({ kind: "text", text: clip(evt.result) });
  }
  return out;
}
  ...
  private runTurn(prompt: string): void {
    ...
    proc.stdout?.on("data", (c: Buffer) => {
      ...
      for (const ln of lines) {
        ...
        if (emit.error) { this.cb.onTrajectory([{ kind: "text", text: "[cursor error] " + clip(emit.error).slice(0, 500) }]); this.cb.onActivity("error", emit.error.slice(0, 200)); }
      }
    });
