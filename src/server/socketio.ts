...
// May this user READ this channel (and thus join its realtime room)? Read access = channel member, OR a public/showcase
...
async function canReadChannel(uid: string, serverId: string, channelId: string): Promise<boolean> {
  ...
  if (ch.type === "channel" || ch.type === "showcase") return true;        // public/showcase: any server member may read realtime events
