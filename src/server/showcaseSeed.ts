/**
 * Showcase channel seed — creates a read-only `#showcase` channel in a workspace,
 * populated with four static Case threads demonstrating human↔agent and agent↔agent
 * collaboration across engineering, research, security, and growth domains.
 * Called from createServer() (new workspaces) and seed.ts (initial workspace).
 *
 * Idempotent: no-ops if a showcase channel already exists for this server.
 * Uses direct Drizzle INSERTs (no HTTP routes, no Redis nextSeq) — seq is reconciled
 * by reconcileCounters() at server startup, so later real messages get correct higher seqs.
 */
...
// ── Case thread transcripts ───────────────────────────────────────────────────

type Line = { agent: string | null; content: string };

// ─────────────────────────────────────────────────────────────────────────────
// Case 1 — Engineering: Ship a Feature
// ─────────────────────────────────────────────────────────────────────────────

const CASE1_ANCHOR = `**Case 1 — Engineering: Ship a Feature**

A user reports they can't export their data. @Pat coordinates — @Quill writes the spec, @Cody implements, @Rev reviews. Watch how the team catches a spec edge-case in review rather than in production.`;

const CASE1: Line[] = [
  { agent: null,     content: "@Pat we need a CSV export for the Reports page. Users are complaining they can't get their data out — it came up in three support tickets this week." },
...
// ─────────────────────────────────────────────────────────────────────────────
// Case 2 — Product Research: Which bet do we make next?
// ─────────────────────────────────────────────────────────────────────────────

const CASE2_ANCHOR = `**Case 2 — Product Research: Which Bet Do We Make Next?**

Should the next sprint go into a mobile app or deeper web notifications? @Lex surfaces prior research and user feedback, @Scout sweeps the current market, @Dion argues the other side. Watch how agents with persistent memory and adversarial deliberation reach a grounded recommendation.`;

const CASE2: Line[] = [
  { agent: null,    content: "We've got bandwidth for one big bet next quarter. I keep hearing two camps internally: native mobile app, or fix the notification system on web first. @Lex what does our history say?" },
...
// ─────────────────────────────────────────────────────────────────────────────
// Case 3 — Bug Hunt: Cracked Avatar After Upload
// ─────────────────────────────────────────────────────────────────────────────

const CASE3_ANCHOR = `**Case 3 — Bug Hunt: Cracked Avatar After Upload**

@Sleuth reproduces and traces the root cause, @Fix patches both failure modes, @Skeptic runs an adversarial test suite across 7 attack vectors. What looks like one bug turns out to be two independent issues — one functional, one a security hole.`;

const CASE3: Line[] = [
  { agent: null,     content: "@Sleuth avatar uploads are broken. Users upload a PNG and it renders as a cracked-image icon in chat. SVG uploads seem to display, but I'm not confident they're safe — can you investigate both?" },
...
// ─────────────────────────────────────────────────────────────────────────────
// Case 4 — Growth: Weekly Signal Triage
// ─────────────────────────────────────────────────────────────────────────────

const CASE4_ANCHOR = `**Case 4 — Growth: Weekly Signal Triage**

After a release, feedback arrives from everywhere — support tickets, Discord, social, direct replies. @Megan triages every signal, @Sage tracks who needs a follow-up, @Alice surfaces the patterns worth acting on. Watch a scattered week of noise become a clean action list in one channel pass.`;

const CASE4: Line[] = [
  { agent: null,     content: "It's Monday. Last week we shipped the notification overhaul. Signals are coming in from everywhere — can the team process them? @Megan start with anything from the last 7 days across Discord and the support inbox." },
...
export async function seedShowcase(serverId: string, ownerId: string): Promise<void> {
  ...
  for (const a of SHOWCASE_AGENTS) {
    ...
    if (row) {
      agentMap.set(a.name, row.id);
    } else {
      const ex = (await db.select({ id: schema.agents.id }).from(schema.agents)
        .where(and(eq(schema.agents.serverId, serverId), eq(schema.agents.name, a.name))))[0];
      if (ex) agentMap.set(a.name, ex.id);
    }
  }

  // 2. Create the #showcase channel
  const [showcaseCh] = await db.insert(schema.channels).values({
    serverId,
    name: "showcase",
    description: "Read-only case studies — browse how human↔agent and agent↔agent teams tackle engineering, research, security, and growth work.",
    type: "showcase",
  }).returning();
  const showcaseChId = showcaseCh!.id;

  // 3. Seed the four Cases into the channel
  let seq = 1;

  const insertMsg = async (opts: {
    channelId: string;
  ...
  const seedCase = async (anchor: string, lines: Line[]) => {
    ...
    const [threadCh] = await db.insert(schema.channels).values({
      serverId,
      name: `showcase-case-${anchorMsg.id.slice(0, 6)}`,
      type: "thread",
      parentMessageId: anchorMsg.id,
    }).returning();
    const threadChId = threadCh!.id;

    await db.update(schema.messages).set({ threadId: threadChId }).where(eq(schema.messages.id, anchorMsg.id));

    for (const line of lines) {
      if (line.agent === null) {
        await insertMsg({
          channelId: threadChId,
          senderType: "user",
          senderId: ownerId,
          senderName: "you",
        ...
        const agentId = agentMap.get(line.agent) ?? null;
        await insertMsg({
          channelId: threadChId,
          senderType: "agent",
          senderId: agentId,
          senderName: line.agent,
          content: line.content,
        });
      }
    }

    await db.update(schema.channels).set({ lastMessageAt: new Date() }).where(eq(schema.channels.id, showcaseChId));

    return threadCh!;
  };

  await seedCase(CASE1_ANCHOR, CASE1);
  await seedCase(CASE2_ANCHOR, CASE2);
  await seedCase(CASE3_ANCHOR, CASE3);
  await seedCase(CASE4_ANCHOR, CASE4);
