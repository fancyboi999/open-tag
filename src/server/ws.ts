...
async function onDaemon(ws: WebSocket, key: string): Promise<void> {
  ...
  ws.on("message", async (data) => {
    ...
    try {
      ...
      else if ((msg.type === "workspace:file_tree" || msg.type === "workspace:file_content" || msg.type === "skills:list" || msg.type === "models" || msg.type === "agent:resource-budget") && msg.requestId) resolveDaemonRequest(msg.requestId, msg);
